...
export class AuthService {
  ...
  async registration(data) {
    const user = await this.userService.getUserByUsrName(data.userName);
    if (user) {
  ...
  async validateUser(userDto): Promise<any> {
    const user = await this.userService.getUserByUsrName(userDto.userName);

    if (!user) {
      throw new HttpException(
        `The user with this username is not exist!`,
        HttpStatus.BAD_REQUEST,
      );
    }
