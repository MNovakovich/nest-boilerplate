...
export class DescriptiveIndicatorService {
  ...
  constructor(@Inject('DESCRIPTIVE_INDICATOR_REPOSITORY') private descriptive_indicatorRepository: typeof DescriptiveIndicator) {}
  ...
  async findAll(query:any): Promise<DescriptiveIndicator[]> {
    const res = await paginateFilterUrl.query(this.descriptive_indicatorRepository, query, {});
    if (!res) throw new NotFoundException();
    return res;
  }

  async findOne(id: number) {
    return await this.descriptive_indicatorRepository.findOne({ where: { id } });
  }

  async update(id: number, data: UpdateDescriptiveIndicatorDto): Promise<any> {
    const result = await this.descriptive_indicatorRepository.findOne({ where: { id } });
    if (!result) {
      return new HttpException('descriptive_indicator not exist!', HttpStatus.BAD_REQUEST);
    }
