...
export class DiagnosisIndicatorService {
  ...
  constructor(
    @Inject('DIAGNOSIS_INDICATOR_REPOSITORY')
    private diagnosis_indicatorRepository: typeof DiagnosisIndicator,
  ) {}
  ...
  async findAll(query: any): Promise<DiagnosisIndicator[]> {
    const res = await paginateFilterUrl.query(
      this.diagnosis_indicatorRepository,
      query,
      {},
    );
  ...
  async update(id: number, data: UpdateDiagnosisIndicatorDto): Promise<any> {
    const result = await this.diagnosis_indicatorRepository.findOne({
      where: { id },
    });
    if (!result) {
      return new HttpException(
        'diagnosis_indicator not exist!',
