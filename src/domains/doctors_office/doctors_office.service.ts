...
export class DoctorsOfficeService {
  ...
  constructor(
    @Inject('DOCTORS_OFFICE_REPOSITORY')
    private doctors_officeRepository: typeof DoctorsOffice,
  ) {}
  ...
  async update(id: number, data: UpdateDoctorsOfficeDto): Promise<any> {
    const result = await this.doctors_officeRepository.findOne({
      where: { id },
    });
    if (!result) {
      return new HttpException(
        'doctors_office not exist!',
