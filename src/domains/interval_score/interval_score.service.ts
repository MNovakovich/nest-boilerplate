...
export class IntervalScoreService {
  ...
  constructor(
    @Inject('INTERVAL_SCORE_REPOSITORY')
    private interval_scoreRepository: typeof IntervalScore,
  ) {}
  ...
  async update(id: number, data: UpdateIntervalScoreDto): Promise<any> {
    const result = await this.interval_scoreRepository.findOne({
      where: { id },
    });
    if (!result) {
      return new HttpException(
        'interval_score not exist!',
