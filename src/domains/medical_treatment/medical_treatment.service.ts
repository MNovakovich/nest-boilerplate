...
export class MedicalTreatmentService {
  ...
  constructor(@Inject('MEDICAL_TREATMENT_REPOSITORY') private medical_treatmentRepository: typeof MedicalTreatment) {}
  ...
  async findAll(query:any): Promise<MedicalTreatment[]> {
    const res = await paginateFilterUrl.query(this.medical_treatmentRepository, query, {});
  ...
  async update(id: number, data: UpdateMedicalTreatmentDto): Promise<any> {
    const result = await this.medical_treatmentRepository.findOne({ where: { id } });
    if (!result) {
      return new HttpException('medical_treatment not exist!', HttpStatus.BAD_REQUEST);
    }
