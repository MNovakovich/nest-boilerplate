...
export class OfficeUserService {
  ...
  constructor(@Inject('OFFICE_USER_REPOSITORY') private office_userRepository: typeof OfficeUser) {}
  ...
  async findAll(query:any): Promise<OfficeUser[]> {
    const res = await paginateFilterUrl.query(this.office_userRepository, query, {});
  ...
  async update(id: number, data: UpdateOfficeUserDto): Promise<any> {
    ...
    if (!result) {
      return new HttpException('office_user not exist!', HttpStatus.BAD_REQUEST);
    }
