...
export class TakecareNoteService {
  ...
  constructor(@Inject('TAKECARE_NOTE_REPOSITORY') private takecare_noteRepository: typeof TakecareNote) {}
  ...
  async findAll(query:any): Promise<TakecareNote[]> {
    const res = await paginateFilterUrl.query(this.takecare_noteRepository, query, {});
  ...
  async update(id: number, data: UpdateTakecareNoteDto): Promise<any> {
    const result = await this.takecare_noteRepository.findOne({ where: { id } });
    if (!result) {
      return new HttpException('takecare_note not exist!', HttpStatus.BAD_REQUEST);
    }
