...
export class TreatmentDiagnosisService {
  ...
  constructor(@Inject('TREATMENT_DIAGNOSIS_REPOSITORY') private treatment_diagnosisRepository: typeof TreatmentDiagnosis) {}
  ...
  async findAll(query:any): Promise<TreatmentDiagnosis[]> {
    const res = await paginateFilterUrl.query(this.treatment_diagnosisRepository, query, {});
  ...
  async update(id: number, data: UpdateTreatmentDiagnosisDto): Promise<any> {
    const result = await this.treatment_diagnosisRepository.findOne({ where: { id } });
    if (!result) {
      return new HttpException('treatment_diagnosis not exist!', HttpStatus.BAD_REQUEST);
    }
