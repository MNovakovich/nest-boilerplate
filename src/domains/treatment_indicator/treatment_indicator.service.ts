...
export class TreatmentIndicatorService {
  ...
  constructor(
    @Inject('TREATMENT_INDICATOR_REPOSITORY')
    private treatment_indicatorRepository: typeof TreatmentIndicator,
  ) {}
  ...
  async findAll(query: any): Promise<TreatmentIndicator[]> {
    const res = await paginateFilterUrl.query(
      this.treatment_indicatorRepository,
      query,
      {},
    );
  ...
  async update(id: number, data: UpdateTreatmentIndicatorDto): Promise<any> {
    const result = await this.treatment_indicatorRepository.findOne({
      where: { id },
    });
    if (!result) {
      return new HttpException(
        'treatment_indicator not exist!',
